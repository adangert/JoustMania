...
import common, piparty, controller_manager
import logging
import setproctitle
import time
...
def main_track_move(menu, restart, move_serial, move_num, menu_opts, game_opts, force_color, battery, dead_count, game_mode, \
                    ...
                    revive, kill_proc, manager):

    # Set the process title
    setproctitle.setproctitle(f"JoustMania-main_track_move({move_serial})")

    controller_manager.use_manager(manager)
    move_controller = controller_manager.Controller(move_serial)

    while not kill_proc.value:
        move_controller.set_rumble(0)
        if restart.value == 1:
            time.sleep(RESTART_SLEEP_SECS)
        elif menu.value == 1:
            logger.debug("Tracking Move (Menu): {}".format(move_serial))
            piparty.track_move(move_serial, move_num, move_controller, menu_opts, force_color, battery, dead_count, restart, menu, kill_proc)
        else:
            ...
            if game_mode.value == common.Games.Tournament.value:
                tournament.Joust.track_move(
                    controller=move_controller, team=team, team_color_enum=team_color_enum, dead_move=dead_move, invincible_move=invincible_move, \
