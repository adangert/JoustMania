...
class Joust(Game):
    ...
    def init_audio(self):
        ...
        if self.play_audio:
            ...
            self.red_power_ready = Audio(
                "audio/Commander/vox/" + self.voice + "/red_power_ready.wav"
            )
            self.blue_power_ready = Audio(
                "audio/Commander/vox/" + self.voice + "/blue_power_ready.wav"
            )
    ...
    def before_game_loop(self):
        ...
        Audio(
            "audio/Commander/vox/" + self.voice + "/commanders chosen.wav"
    ...
    def check_winner(self):
        for commander in self.current_commander:
            if self.dead_moves[commander].value == Status.DEAD.value:
                self.winning_team = (
                    self.teams[commander] + 1
                ) % 2  # Winning team is the other team
                return True
        return False
    ...
    def check_commander_select(self):
        ...
        for move_serial in self.moves:
            if (
                self.opts[move_serial][Opts.SELECTION.value] == Selection.COMMANDER_READY.value
                and self.opts[move_serial][Opts.HOLDING.value]
            ):
    ...
    def change_random_commander(self, team, exclude_commander=None):
        ...
        logger.debug(f"Team move serials: {team_move_serials}")
    ...
    def update_team_powers(self):
        for team in [Team.ALPHA.value, Team.BRAVO.value]:
            ...
            self.powers_percent[team] = max(
                min(
                    (time.time() - self.overdrive_start_time[team])
                    / (self.time_to_power[team] * 1.0),
                    1.0,
                ),
                0.0,
            )
            ...
            if self.overdrive_status[team] == Overdrive.CHARGING.value:
                if self.powers_percent[team] >= 1.0:
                    logger.debug(f"Power activated for team: {team}")
                    self.overdrive_status[team] = Overdrive.READY.value
                    self.opts[self.current_commander[team]][Opts.OVERDRIVE.value] = (
                        Overdrive.READY.value
                    )
                    ...
                    self.opts[self.current_commander[team]][Opts.POWER_PERCENT.value] = int(
                        self.powers_percent[team] * 100
                    )
    ...
    def overdrive(self, team):
        ...
        logger.debug(f"Overdrive active for: {team}")
        self.overdrive_sfx.start_effect()

        # Turn on overdrive for all controllers on this team
        for move_serial in self.moves:
            if self.teams[move_serial] == team:
                logger.debug(f"Activating overdrive for: {move_serial}")
        ...
        for team in [Team.ALPHA.value, Team.BRAVO.value]:
            if self.overdrive_status[team] == Overdrive.ACTIVE.value:
                if time.time() >= self.overdrive_end_time[team]:
                    logger.debug(f"Overdrive ended for team: {team}")
                    self.overdrive_status[team] = Overdrive.CHARGING.value
                    # Turn off overdrive for all moves on this team
                    for move_serial in self.moves:
                        if self.teams[move_serial] == team:
                            logger.debug(f"Deactivating overdrive for: {move_serial}")
                            self.opts[move_serial][Opts.OVERDRIVE.value] = Overdrive.CHARGING.value
        ...
        while opts[Opts.INTRO.value] == Intro.ON.value:
            if move.poll():
                ...
                if (
                    ...
                    and not opts[Opts.HOLDING.value]
                    and opts[Opts.COMMANDER_SET.value]
                ):
    ...
    def get_warning(cls, team, speed_percent, sensitivity, opts):
        ...
        if (
            not opts[Opts.IS_COMMANDER.value]
            and opts[Opts.OVERDRIVE.value] == Overdrive.ACTIVE.value
        ):
    ...
    def handle_opts(cls, move, team, opts, dead_move):
        if opts[Opts.IS_COMMANDER.value] and opts[Opts.OVERDRIVE.value] == Overdrive.READY.value:
            ...
            if (
                opts[Opts.OVERDRIVE.value] == Overdrive.READY.value
                and not opts[Opts.HOLDING.value]
    ...
    def handle_team_color(cls, move, team, opts, team_color):
        if not opts[Opts.IS_COMMANDER.value]:
            ...
            return team_color
        return cls.calculate_flash_time(
            *CURRENT_COMMANDER_COLORS[team].value, opts[Opts.POWER_PERCENT.value] / 100
        )
