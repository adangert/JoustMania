...
class Joust(Game):
    ...
    def init_moves(self):
        ...
        for move_num, move_serial in enumerate(self.moves):
            self.dead_moves[
                move_serial
            ].value = Status.ON.value  # Aren't going to be using accelerometer
    ...
    def before_game_loop(self):
        ...
        logger.debug(f"Initial bomb serial: {self.bomb_serial}")
    ...
    def handle_status(self):
        for move_serial in self.moves:
            if self.dead_moves[move_serial].value == Status.DIED.value:
                logger.debug(
                    f"Move is dead: {move_serial}, has bomb: {self.opts[move_serial][Opts.HAS_BOMB.value]}"
                )
        ...
        if self.opts[self.bomb_serial][Opts.SELECTION.value] == Selection.NOTHING.value:
            ...
            self.opts[self.bomb_serial][Opts.SELECTION.value] == Selection.NEXT_BUTTON.value
        ...
        if time.time() > self.bomb_time:
            logger.debug(f"Bomb exploded: {time.time()}")
    ...
    def move_bomb(self):
        ...
        self.opts[self.bomb_serial][Opts.HAS_BOMB.value] = True
        self.opts[self.bomb_serial][Opts.BOMB_START_TIME.value] = self.opts[old_bomb_serial][
            Opts.BOMB_START_TIME.value
        ]
        self.opts[self.bomb_serial][Opts.BOMB_END_TIME.value] = self.opts[old_bomb_serial][
            Opts.BOMB_END_TIME.value
        ]

        logger.debug(f"Moving bomb to: {self.bomb_serial}")
    ...
    def get_bomb_length(self):
        ...
        if self.bomb_length < 1:
            self.bomb_length = 1
        logger.debug(f"New bomb length: {self.bomb_length}")
    ...
    def check_faked_out(self):
        ...
        for move_serial in self.moves:
            if self.dead_moves[move_serial].value == Status.ON.value:
                ...
                if (
                    self.opts[move_serial][Opts.SELECTION.value] == Selection.TRIGGER.value
                    and not self.opts[move_serial][Opts.HOLDING.value]
                ):
                    self.opts[move_serial][Opts.HOLDING.value] = True
                    victim = self.get_next_serial(move_serial)
                    logger.debug(f"{move_serial} is trying to fake out {victim}")
                    self.reset_bomb_time(True)
                ...
                if self.opts[move_serial][Opts.FAKED.value] in [
                    Faked.ATTEMPT.value,
                    Faked.FAKED.value,
                ]:
                    ...
                    if self.opts[move_serial][Opts.FAKED.value] == Faked.FAKED.value:
                        ...
                        logger.debug(f"Killed from fake bomb: {faker}")

                        self.reset_bomb_time(True)
                        self.move_bomb()

                    elif (
                        self.opts[move_serial][Opts.FAKED.value] == Faked.ATTEMPT.value
                        and self.opts[move_serial][Opts.SELECTION.value] == Selection.COUNTER.value
                    ):
                        if self.play_audio:
                            self.explosion40.start_effect()
                        ...
                        logger.debug(f"Killed from from counter: {faker}")
    ...
    def kill_player(self, dead_move):
        ...
        logger.debug(
            f"Lost a life: {dead_move}, {self.opts[dead_move][Opts.LIVES.value]} remaining"
        )
        ...
        logger.debug(
            f"Done killing player: {dead_move}, has bomb: {self.opts[dead_move][Opts.HAS_BOMB.value]}"
        )
    ...
    def get_next_random_holder(self):
        if self.next_rand_holder == self.bomb_serial:
            ...
            while (
                ...
                or not self.dead_moves[self.next_rand_holder].value == Status.ON.value
            ):
                self.next_rand_holder = self.move_serials[random.choice(range(len(self.moves)))]
    ...
    def reset_bomb_time(self, reset_length):
        ...
        self.opts[self.bomb_serial][Opts.BOMB_START_TIME.value] = Joust.time_to_ms_int(
            self.bomb_start_time
        )
    ...
    def handle_team_color(cls, move, team, opts, team_color):
        ...
        if opts[Opts.SELECTION.value] == Selection.TRIGGER.value:
            ...
            if opts[Opts.TRIGGER.value] <= 127:
                return cls.calculate_bomb_color(
                    fake_bomb_color, no_fake_bomb_color, opts[Opts.TRIGGER.value]
                )
            if opts[Opts.TRIGGER.value] > 127:
                return cls.calculate_bomb_color(
                    no_fake_bomb_color, fake_bomb_color, opts[Opts.TRIGGER.value]
                )
        else:
            if opts[Opts.BOMB_START_TIME.value] > 0:
                time_ms = cls.time_to_ms_int(time.time())
                percentage = 1 - (
                    (opts[Opts.BOMB_END_TIME.value] - time_ms)
                    / (opts[Opts.BOMB_END_TIME.value] - opts[Opts.BOMB_START_TIME.value])
                )
            else:
                percentage = 1
    ...
    def handle_opts(cls, move, team, opts, dead_move=None):
        if move.poll():
            ...
            if (
                opts[Opts.FAKED.value] == Faked.ATTEMPT.value
                and not opts[Opts.HAS_BOMB.value]
                and (trigger > 50 or (pressed & common.Button.MIDDLE.value))
            ):
                logger.debug(f"Faked out: {move.get_serial()}")
            ...
            # If pressing counter while being faked, counter
                ...
                not opts[Opts.HAS_BOMB.value]
                and (
                    pressed & common.Button.SQUARE.value
                    or pressed & common.Button.TRIANGLE.value
                    or pressed & common.Button.CIRCLE.value
                    or pressed & common.Button.CROSS.value
                )
                and opts[Opts.FAKED.value] == Faked.ATTEMPT.value
            ):
                logger.debug(f"Pressed Counter Button: {move.get_serial()}")
                opts[Opts.SELECTION.value] = Selection.COUNTER.value
            # If pressing counter while NOT being faked, die
            elif (
                not opts[Opts.FAKED.value] == Faked.ATTEMPT.value
                and not opts[Opts.HAS_BOMB.value]
                and (
                    pressed & common.Button.SQUARE.value
                    or pressed & common.Button.CIRCLE.value
