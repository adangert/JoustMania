...
class Joust(Game):
    ...
    def generate_random_teams(self, num_teams, num_moves):
        ...
        logger.debug(f"Number of traitors: {num_traitors}")
        ...
        while num_traitors > 0:
            ...
            serial = random.choice(
                [serial for serial in copy_serials if self.teams[serial] == team_choice]
            )
            copy_serials.remove(serial)
            logger.debug(f"Teams list: {copy_teams}")
            ...
            num_traitors -= 1
            logger.debug(f"Traitor selected for team {team_choice}: {serial}")

        logger.debug(f"Final Teams list: {self.teams}")
    ...
    def before_game_loop(self):
        ...
        for move_serial, dead in self.dead_moves.items():
            logger.debug(
                f"Move {move_serial} on team {self.teams[move_serial]}: {dead.value}"
            )
    ...
    def count_down(self):
        ...
        for move_serial in self.traitors:
            logger.debug(f"Setting to alive: {move_serial}")
