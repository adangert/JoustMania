...
class Joust(Game):
    ...
    def handle_status(self):
        ...
        for move_serial in self.humans:
            ...
            if self.dead_moves[move_serial].value == Status.DIED.value:
                logger.debug(f"Human has died, switching to zombie: {move_serial}")
                if self.play_audio:
            ...
            # Pistol fired (Shoot one random zombie)
                ...
                logger.debug(f"Human has fired pistol: {move_serial}")
                if self.play_audio:
            ...
            # Molotov fired (Shoot all alive zombies)
                ...
                logger.debug(f"Human has thrown molotov: {move_serial}")
                if self.play_audio:
        ...
        for move_serial, spawn_time in self.zombies.items():
            # If a zombie died
            if self.dead_moves[move_serial].value == Status.DIED.value:
                logger.debug(f"Zombie has died by melee: {move_serial}")
                if self.play_audio:
                    self.play_death_sound(move_serial)
            ...
            # If zombie is ready to revive
                ...
                logger.debug(f"Zombie has revived: {move_serial}")
                if self.play_audio:
    ...
    def kill_zombie(self, move_serial):
        if move_serial not in self.shot_zombies:
            self.dead_moves[move_serial].value = Status.DEAD.value
        self.zombies[move_serial] = (
            time.time() + 2
        )  # self.get_respawn_time() TODO - reimplement this
    ...
    def shoot_zombies(self, targets, reward_odds):
        kill_zombie = False
        logger.debug(f"All Zombies: {self.zombies}")
        alive_zombies = [key for key, value in self.zombies.items() if value is None]
        logger.debug(f"Alive Zombies: {alive_zombies}")
        ...
        for i in range(targets):
            ...
            if len(alive_zombies) > 0:
                ...
                logger.debug(f"Zombie has died by gun: {zombie_serial}")
    ...
    def handle_opts(cls, move, team, opts, dead_move):
        ...
        if team != -1:
            ...
            if (
                not opts[Opts.HOLDING.value]
                and 0 < opts[Opts.AMMO.value] < 5
