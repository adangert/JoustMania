...
def darken_color(color_array: tuple[int, ...], factor: float) -> tuple[int, ...]:
...
def generate_team_colors(
    ...
    if num_teams == 3:
        ...
        team3 = random.choice(team3_allowed) if team3_allowed else Colors.White
    ...
    if num_teams == 4:
        ...
        return teams
    # num_teams > 4: set colors, we're in FFA territory now
    sets_needed = (num_teams // 8) + 1
    return (ordered_color_list * sets_needed)[:num_teams]


def change_color(color_array: list[int], r: int, g: int, b: int) -> None:
    color_array[0] = r
    color_array[1] = g
    color_array[2] = b
