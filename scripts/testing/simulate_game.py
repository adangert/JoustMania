...
async def simulate_ffa_game(num_controllers: int, duration: int, host: str = "localhost"):
    ...
    # Start game
    game_response = await game_client.StartGame(game_coordinator_pb2.StartGameRequest(mode="FFA"))
    ...
    try:
        while time.time() - start_time < duration and len(alive_controllers) > 1:
            ...
            if random.random() < 0.05 and len(alive_controllers) > 1:  # 5% chance
                ...
                if response.success:
                    ...
                    print(
                        f"💀 Controller {controller_idx} died! (accel: {response.accel_magnitude:.2f})"
                    )
        ...
        print(
            f"🏁 Game ending. Winner: Controller {winner}"
            if winner is not None
            else "🏁 Game ending. No winner"
        )
...
async def simulate_teams_game(
    num_controllers: int, num_teams: int, duration: int, host: str = "localhost"
):
    ...
    print(
        f"🎮 Starting Teams game simulation with {num_controllers} controllers, {num_teams} teams"
    )

    # Start game
    game_response = await game_client.StartGame(game_coordinator_pb2.StartGameRequest(mode="Teams"))
        ...
        while time.time() - start_time < duration and len(alive_controllers) > 1:
            ...
            if random.random() < 0.05 and len(alive_controllers) > 1:  # 5% chance
                ...
                if response.success:
                    ...
                    print(
                        f"💀 Controller {controller_idx} (Team {team_num}) died! (accel: {response.accel_magnitude:.2f})"
                    )
                    print(f"   {len(alive_controllers)} controllers remaining")
...
async def main():
    ...
    parser.add_argument(
        "--mode", default="FFA", choices=["FFA", "Teams", "RandomTeams"], help="Game mode"
    )
    parser.add_argument("--controllers", type=int, default=4, help="Number of controllers")
    parser.add_argument("--teams", type=int, default=2, help="Number of teams (for Teams mode)")
